...
function renderProgressively() {
  ...
  if(gRayMarchScene.children.length < renderNum){
    return;
  }
  ...
  gRayMarchScene.children.forEach(function(child){
    ...
    child.material.uniforms['stepMult']['value'] = gStepMult;
  });

  gRenderer.render(gRayMarchScene, gLowResBlitCamera);
  gRenderer.render(scene1, gCamera);
  //
  ...
  if (cameraMoved) {
    gFrameIndex = 0;
    emaAlpha = 1.0;
    console.log("positions",gCamera.position);
    console.log("dir",gOrbitControls.target.x);
    console.log("dir",gOrbitControls.target.y);
    console.log("dir",gOrbitControls.target.z);
    dynamicLoading();
  }
