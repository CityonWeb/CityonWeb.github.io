...
void main() {
  ...
  mat4 world_to_block =  global2local;
  float ground_y = -0.10;
  // intersect with xz plane
  float tXZ = (ground_y - vOrigin.y) / vDirection.y;
  vec4 xy_min = vec4(aabbmin_xy,1.0);
  vec4 xy_max = vec4(aabbmax_xy,1.0);
  vec3 xz = vOrigin + tXZ * vDirection;
  // verify that the intersection is within the image plane
  if(xz.x <= xy_min.x || xz.x >= xy_max.x || xz.z <= xy_min.z || xz.z >= xy_max.z) {
    discard;
  }
  vec3 originWorld = (world_to_block * vec4(vOrigin, 1.0)).xyz;
  vec3 directionWorld = normalize((world_to_block * vec4(vDirection, 0.0)).xyz);
  ...
  int step = 0;
  vec3 t_accumulated = vec3(0.0, 0.0, 0.0);
  ...
  while (step < maxStep && visibility > 1.0 / 255.0) {
    ...
    vec3 z_real = inverseContract(z);
    if (!(all(greaterThanEqual(z, aabbmin)) && all(lessThanEqual(z, aabbmax)))) {
        tContracted += stepSizeContracted;
        continue; 
    }
    // vec3 t_word_vec = (inverseContract(z)-originWorld)/directionWorld;
    // float t_word = (t_word_vec.x+t_word_vec.y+t_word_vec.z)/3.0;
  ...
  vec3 pos_word = originWorld + t_accumulated * directionWorld;
  
  gl_FragColor = vec4(accumulatedColor, sum_w);
  // if(visibility > 0.95){
